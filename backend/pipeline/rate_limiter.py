...
class GroqRotatingClient:
    def __init__(self):
        ...
        self.clients = [Groq(api_key=k, max_retries=0) for k in GROQ_API_KEYS]
    ...
    def _rate_check(self):
        ...
        if elapsed > 60:
            self.request_count = 0
            self.window_start = time.time()
        if self.request_count >= (GROQ_RPM_LIMIT * len(self.clients)):
            wait = 60 - elapsed + 2
    ...
    def chat(self, messages: list, temperature: float = 0.2, max_tokens: int = 2000) -> str:
        ...
        """
        for attempt in range(max(MAX_RETRIES, 6)):
            try:
                self._rate_check()
